...
## Use scripts version install2.r if it exists
if [ -f "/init_scripts/bin/install2.r" ]; then
    ln -sf /init_scripts/bin/install2.r /usr/local/bin/install2.r
else
    ln -sf "${R_HOME}/site-library/littler/examples/install2.r" /usr/local/bin/install2.r
fi

## Install R libraries listed in r_libs if it exists
if [ -f "/init_scripts/r_libs" ]; then
    r_libs=$(</init_scripts/r_libs)
    install2.r ${r_libs}
fi
